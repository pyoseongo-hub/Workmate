...
export default function Home() {
  ...
  const rejectEmployeeSwap = async () => { try { if (workspace?.id && pendingSwaps[0]) await confirmSwap.mutateAsync({ id: pendingSwaps[0].id, workspaceId: workspace.id, confirmed: false }); setNotice("교대 요청을 거절했습니다."); } catch (caught) { setNotice(caught instanceof Error ? caught.message : "교대 거절에 실패했습니다."); } };
  const share = async () => {
    ...
    <div className="min-h-screen bg-[#f6f8fb] text-slate-900">
      <header className="sticky top-0 z-30 border-b border-slate-200/80 bg-white/90 backdrop-blur-xl">
        <div className="mx-auto flex h-[72px] max-w-[1440px] items-center justify-between px-4 sm:px-7 lg:px-10">
          ...
          <div className="flex items-center gap-2 sm:gap-3"><div className="hidden items-center gap-2 rounded-full border border-slate-200 bg-slate-50 px-3 py-2 text-xs font-semibold text-slate-600 sm:flex"><span className="h-2 w-2 rounded-full bg-emerald-500" />{roleLabel}</div><button onClick={() => setShowNotificationCenter(!showNotificationCenter)} className="relative rounded-xl p-2.5 text-slate-500 hover:bg-slate-100" aria-label="알림"><Bell className="h-[18px] w-[18px]" />{pendingCount > 0 && <span className="absolute right-1.5 top-1.5 h-2 w-2 rounded-full bg-rose-500 ring-2 ring-white" />}</button><div className="hidden h-8 w-8 items-center justify-center rounded-full bg-blue-100 text-xs font-bold text-blue-700 sm:flex">{owner ? '김' : '서'}</div></div>
        </div>
      </header>
      <div className="mx-auto flex max-w-[1440px]">
        <aside className={`${showMenu ? 'flex' : 'hidden'} fixed inset-y-[72px] left-0 z-20 w-[250px] flex-col border-r border-slate-200 bg-white p-5 lg:sticky lg:top-[72px] lg:flex lg:h-[calc(100vh-72px)] lg:shrink-0`}>
          ...
          <nav className="space-y-1">{navItems.map((item, index) => <button key={item} onClick={() => { if (item === '직원 관리' && !owner) { setShowOwnerAuth(true); return; } setActive(item); setShowMenu(false); }} className={`flex w-full items-center gap-3 rounded-xl px-3 py-3 text-left text-sm font-semibold transition ${active === item || (active === '대시보드' && index === 0) ? 'bg-blue-50 text-blue-700' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-900'}`}>{index === 0 ? <Sparkles className="h-[17px] w-[17px]" /> : index === 1 ? <CalendarDays className="h-[17px] w-[17px]" /> : index === 2 ? <FileClock className="h-[17px] w-[17px]" /> : index === 3 ? <UsersRound className="h-[17px] w-[17px]" /> : <UserRound className="h-[17px] w-[17px]" />}<span>{item}</span>{item === '교대 승인' && pendingCount > 0 && <span className="ml-auto flex h-5 min-w-5 items-center justify-center rounded-full bg-amber-400 px-1.5 text-[10px] font-bold text-white">{pendingCount}</span>}{item === '직원 관리' && owner && <LockKeyhole className="ml-auto h-4 w-4 text-amber-600" />}</button>)}</nav>
        ...
        <main className="min-w-0 flex-1 px-4 py-6 sm:px-7 lg:px-10 lg:py-9">
          <div className="mx-auto max-w-[1120px] space-y-6">
            <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><p className="text-sm font-semibold text-blue-600">좋은 아침이에요, {owner ? '사장님' : '서연님'}</p><h1 className="mt-1 text-[30px] font-extrabold tracking-[-0.06em] sm:text-[38px]">{active === '근무표' || active === '내 근무표' ? '이번 달 근무표' : active}</h1><p className="mt-2 text-sm text-slate-500">기본 근무일자와 확정된 교대 내용을 한눈에 확인하세요.</p></div><div className="flex items-center gap-2">{owner && <Button onClick={() => setShowScheduleDialog(true)} className="h-10 gap-2 rounded-xl bg-slate-900 px-4 text-xs font-bold shadow-lg shadow-slate-900/15 hover:bg-slate-800"><Plus className="h-4 w-4" />근무일자 등록</Button>}</div></div>
            ...
            {active === '직원 관리' && owner && <Card className="overflow-hidden rounded-3xl border-0 shadow-sm shadow-slate-200/60"><CardHeader className="border-b border-slate-100 px-5 py-5 sm:px-7"><div className="flex items-center justify-between"><div><CardTitle className="text-lg font-extrabold tracking-tight">직원 관리</CardTitle><p className="mt-1 text-xs text-slate-400">매장 직원을 추가, 수정, 삭제할 수 있어요.</p></div><Button onClick={() => { const name = prompt("직원 이름을 입력하세요:"); if (name) { addMember.mutateAsync({ workspaceId: workspace?.id || 0, displayName: name }).then(() => { setNotice(`${name} 직원을 추가했습니다.`); utils.members.list.invalidate({ workspaceId: workspace?.id || 0 }); }); } }} className="h-10 gap-2 rounded-xl bg-blue-600 text-xs font-bold hover:bg-blue-700"><Plus className="h-4 w-4" />직원 추가</Button></div></CardHeader><CardContent className="p-5 sm:p-7"><div className="space-y-2">{members.map((member) => <div key={member.id} className="flex items-center justify-between rounded-xl border border-slate-200 bg-slate-50 p-4"><div><p className="font-bold text-slate-900">{member.displayName}</p><p className="text-xs text-slate-500">{member.memberRole === 'owner' ? '사장님' : '직원'}</p></div><div className="flex gap-2">{member.memberRole !== 'owner' && <><Button onClick={() => { const newName = prompt("새로운 이름:", member.displayName); if (newName && newName !== member.displayName) { updateMember.mutateAsync({ workspaceId: workspace?.id || 0, memberId: member.id, displayName: newName }).then(() => { setNotice(`${member.displayName}을 ${newName}으로 변경했습니다.`); utils.members.list.invalidate({ workspaceId: workspace?.id || 0 }); }); } }} variant="outline" size="sm" className="text-xs">수정</Button><Button onClick={() => { if (confirm(`${member.displayName} 직원을 삭제하시겠습니까?`)) { deleteMember.mutateAsync({ workspaceId: workspace?.id || 0, memberId: member.id }).then(() => { setNotice(`${member.displayName} 직원을 삭제했습니다.`); utils.members.list.invalidate({ workspaceId: workspace?.id || 0 }); }); } }} variant="outline" size="sm" className="text-xs text-rose-600 hover:bg-rose-50">삭제</Button></> || <span className="text-xs font-bold text-slate-500">소유자</span>}</div></div>)}</div></CardContent></Card>}
            ...
            {showOwnerAuth && <div className="fixed inset-0 z-50 flex items-end justify-center bg-slate-950/30 p-0 backdrop-blur-sm sm:items-center sm:p-4"><div className="w-full max-w-md rounded-t-[28px] bg-white p-6 shadow-2xl sm:rounded-[28px]"><div className="flex items-start justify-between"><div><h2 className="text-lg font-extrabold tracking-tight text-slate-900">사장님 인증</h2><p className="mt-1 text-xs leading-5 text-slate-400">직원 관리는 사장님 비밀번호가 필요합니다.</p></div><button onClick={() => { setShowOwnerAuth(false); setOwnerPassword(""); setPasswordError(""); }} className="rounded-xl p-2 text-slate-400 hover:bg-slate-100" aria-label="닫기"><X className="h-4 w-4" /></button></div><div className="mt-6 space-y-4"><label className="block"><span className="text-xs font-bold text-slate-600">사장님 비밀번호 (4자리 숫자)</span><input type="password" value={ownerPassword} onChange={(e) => { setOwnerPassword(e.target.value); setPasswordError(""); }} onKeyDown={(e) => { if (e.key === 'Enter' && ownerPassword.length === 4) { if (ownerPassword === OWNER_PIN) { setOwner(true); setActive('직원 관리'); setShowOwnerAuth(false); setOwnerPassword(""); setNotice('사장님 모드로 진입했습니다.'); } else { setPasswordError('잘못된 비밀번호입니다.'); setOwnerPassword(""); } } }} maxLength={4} inputMode="numeric" className="mt-2 h-14 w-full rounded-xl border border-slate-200 px-4 text-2xl font-bold tracking-widest text-center outline-none focus:border-blue-500" placeholder="••••" />{passwordError && <p className="text-xs font-semibold text-rose-600">{passwordError}</p>}</label><p className="text-[11px] text-slate-400">기본 비밀번호: 0000</p><div className="flex gap-2 pt-2"><Button variant="outline" onClick={() => { setShowOwnerAuth(false); setOwnerPassword(""); setPasswordError(""); }} className="h-11 flex-1 rounded-xl text-sm font-bold">취소</Button><Button onClick={() => { if (ownerPassword === OWNER_PIN) { setOwner(true); setActive('직원 관리'); setShowOwnerAuth(false); setOwnerPassword(""); setNotice('사장님 모드로 진입했습니다.'); } else { setPasswordError('잘못된 비밀번호입니다.'); setOwnerPassword(""); } }} disabled={ownerPassword.length !== 4} className="h-11 flex-1 rounded-xl bg-slate-900 text-sm font-bold hover:bg-slate-800">확인</Button></div></div></div></div>}
