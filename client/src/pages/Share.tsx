...
/**
 * 교대 내용 공유 화면 (겉모습만 만들어 둔 것).
 *
 * ⚠️ 아직 실제 자료와 이어지지 않았습니다. 안에 보이는 날짜·이름은 예시입니다.
 *    이어 붙일 때는 주소의 id 로 교대 건을 찾아 실제 값을 넣어야 합니다.
 *
 * ⏱️ 휴게시간은 적지 않습니다 (사장님 방침, 2026-09-01).
 *    앱은 근무시간만 보여 주고, 쉬는 시간·급여 셈은 사장님 몫입니다.
 */
  ...
  return (
    <div className="min-h-screen bg-[#f6f8fb] px-4 py-10 text-slate-900 sm:px-6">
      <div className="mx-auto max-w-lg">
        ...
        <div className="rounded-[28px] bg-slate-900 p-6 text-white shadow-2xl shadow-slate-900/15 sm:p-8"><span className="inline-flex items-center gap-1.5 rounded-full bg-emerald-400/15 px-3 py-1.5 text-[11px] font-bold text-emerald-300"><ShieldCheck className="h-3.5 w-3.5" />상대 확인 완료 · 사장님 승인 대기</span><h1 className="mt-5 text-2xl font-extrabold tracking-tight">교대 내용을 확인해 주세요</h1><p className="mt-2 text-sm leading-6 text-slate-400">전화나 카카오톡으로 합의한 내용을 앱에 등록했습니다. 아래 내용을 확인한 뒤 앱에서 응답해 주세요.</p><div className="mt-7 rounded-2xl bg-white/10 p-5"><div className="flex items-center gap-3 text-blue-200"><Clock3 className="h-5 w-5" /><span className="text-sm font-bold">8월 15일 금요일 · 14:00–20:00</span></div><div className="my-5 h-px bg-white/10" /><div className="flex items-center justify-between"><div><p className="text-xs text-slate-400">기존 근무자</p><p className="mt-1 text-lg font-extrabold">김민수</p></div><div className="text-xl text-slate-500">→</div><div className="text-right"><p className="text-xs text-slate-400">교대 근무자</p><p className="mt-1 text-lg font-extrabold">이서연</p></div></div></div><div className="mt-5 flex items-center gap-2 text-xs text-slate-400"><UsersRound className="h-4 w-4" />홀 서빙 · 앱에서 최종 승인</div></div>
